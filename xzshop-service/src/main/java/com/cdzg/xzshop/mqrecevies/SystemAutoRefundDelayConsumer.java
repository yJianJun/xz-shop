...
@Component
@RabbitListener(queues = RabbitMQConfig.DELAY_SYSTEM_AUTO_REFUND_QUEUE)
@Slf4j
public class SystemAutoRefundDelayConsumer implements ChannelAwareMessageListener {
    ...
    public void onMessage(Long refundOrderId, Message message, Channel channel) throws Exception {
        try {
            ...
            //消息,重试时间一分钟
            rabbitmqUtil.sendSystemAutoRefundDelayMessage(refundOrderId, 60000);
            //确认消息
            channel.basicAck(message.getMessageProperties().getDeliveryTag(), false);
        }
    }
