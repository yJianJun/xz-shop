...
import com.cdzg.xzshop.filter.auth.FilterConstants;
import com.cdzg.xzshop.filter.auth.LoginConstants;
import com.cdzg.xzshop.filter.auth.RequestUtil;
import com.cdzg.xzshop.filter.auth.SpringSessionUtils;
 ...
 * @version 1.0.0
 */
@WebFilter(urlPatterns = {"/admin/*"}, filterName = "filter2LoginAdmin")
@Order(-1)
public class Filter2LoginAdmin implements Filter {

    private static final Logger LOG = LoggerFactory.getLogger(Filter2LoginAdmin.class);
    ...
    public void init(FilterConfig filterConfig) throws ServletException {
        Filter2LoginAdmin.stringSet = OAuthIgnoreUtils.getWebOAuthod("com.cdzg");
    }
...
//        //需要验证的数据
        ...
        Object o=redisTemplate.opsForValue().get("adminToken:"+InterfaceToken);
        if(o==null){
            HttpServletResponse responses = SpringSessionUtils.getResponse();
            RequestUtil.writeJsonToResponseCos(ApiResponse.buildResponse(5003,null,"登录已过期！请重新登录"),
                    SpringSessionUtils.getRequest().getHeader("origin"), responses);
            return;
        }
        UserLoginResponse userLoginResponse=objectMapper.convertValue(o,UserLoginResponse.class);
        SpringSessionUtils.setSession(LoginConstants.Session.WEBAPI_SESSION_ADMIN,userLoginResponse);
        chain.doFilter(request, response);
    }
