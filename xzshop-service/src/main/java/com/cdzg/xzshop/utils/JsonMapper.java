...
* 作       者：bbliu
* 功能描述：简单封装Jackson，实现JSON String<->Java Object的Mapper. 封装不同的输出风格,使用不同的builder函数创建实例.
**************************************************************/
...
public class JsonMapper extends ObjectMapper {
	...
	public static JsonMapper getInstance(Include include) {
		if (mapper == null) {
			mapper = new JsonMapper(include);
		}
		return mapper;
	}

	public static JsonMapper getAlwaysInstance() {
		if (mapper == null) {
			mapper = new JsonMapper(Include.ALWAYS);
		}
		return mapper;
